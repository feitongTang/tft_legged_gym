...
class Cfg(BaseConfig):
    class env:
        ...
        num_scalar_observations = 42    # 我理解这个参数虽然被定义了但是实际并未使用
        # if not None a privilige_obs_buf will be returned by step() (critic obs for assymetric training). None is returned otherwise
        num_privileged_obs = 18         # 我理解这里的privileged obs指的是相比于本身的observation额外的信息
        privileged_future_horizon = 1   # 我理解这个参数虽然被定义了但是实际并未使用
        num_actions = 12
        ...
        # 下面这些变量是控制观测空间包含哪些信息
        # 如果某个判断为True，则会在obs buffer中加入该信息
        observe_vel = True
    ...
    class commands:
        ...
        max_reverse_curriculum = 1.         # 我理解没有用到该参数
        max_forward_curriculum = 1.         # 我理解没有用到该参数
        yaw_command_curriculum = False      # 我理解没有用到该参数
        max_yaw_curriculum = 1.             # 我理解没有用到该参数
        exclusive_command_sampling = False  # 我理解没有用到该参数
        num_commands = 3
        resampling_time = 10.  # time before command are changed[s]
        subsample_gait = False              # 我理解没有用到该参数
